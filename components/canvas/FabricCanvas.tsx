...
export const FabricCanvas = ({
  ...
  const { selectedText, notes, setCurrentPage, scale, currentPage } = useSettings();
  ...
  const removeCanvasContent = (objectId) => {
  ...
  useEffect(() => {
      ...
      fabric.util.enlivenObjects(
        ...
        (objects) => {
          objects.forEach((obj, index) => {
            ...
            const existingObject = fabricCanvas.current?.getObjects().find(item => item.objectId === objectId);
    ...
    if (canvas) {
      ...
      canvas.on("object:removed", (options) => removeCanvasContent(options.target?.objectId));
  ...
  }, [newObject, currentPage,initialContent]);
